...
class Slider:
    def __init__(self):
        ...
        slider_model_path = os.path.join(
            root_dir, "captcha_recognizer", "models", "slider.onnx"
        )
    ...
    def letterbox(
        ...
        new_unpad = (
            max(1, min(new_unpad[0], new_shape[1])),
    ...
    def postprocess(
        ...
        for i, pred in enumerate(preds):
            ...
            masks = self.process_mask(
                protos[i], pred[:, 6:], pred[:, :4], img.shape[:2]
            )
    ...
    def masks_to_segments(
        masks: Union[np.ndarray,], strategy: str = "largest"
    ) -> List[np.ndarray]:
        """
    ...
    @staticmethod
    def draw_segments(
        image, boxes, masks, mask_alpha=0.5, box_thickness=2, draw_labels=True
    ):
        """
        在图像上绘制预测框和掩膜
        ...
        if boxes is not None:
            for box in boxes:
                ...
                # 绘制矩形框
                cv2.rectangle(
                    output, (int(x1), int(y1)), (int(x2), int(y2)), color, box_thickness
                )
    ...
    def image_to_array(source: Union[str, Path, bytes, np.ndarray] = None):
        if isinstance(source, (str, Path)):
            ...
            raise TypeError(
                "Unsupported source type. Only str, Path, bytes, or numpy.ndarray are supported."
            )
    ...
    def pick_out_mask(self, boxes: list, segments):
        ...
        for index, box in enumerate(box_sample):
            if (
                self.y_iou([box_slider[1], box_slider[3]], [box[1], box[3]])
                > Y_IOU_THRESHOLD
            ):
    ...
    def scale_boxes(
        ...
        ratio_pad: Union[Tuple, None] = None,
        padding: bool = True,
        xywh: bool = False,
    ):
        """
    ...
    def batch_probiou(
        self, obb1: np.ndarray, obb2: np.ndarray, eps: float = 1e-7
    ) -> np.ndarray:
        """
        ...
        x2, y2 = (
            np.expand_dims(x.squeeze(-1), 0)
            for x in np.split(obb2[..., :2], 2, axis=-1)
        )
        a1, b1, c1 = self.get_covariance_matrix(obb1)
        a2, b2, c2 = (
            np.expand_dims(x.squeeze(-1), 0) for x in self.get_covariance_matrix(obb2)
        )
    ...
    def nms_rotated(
        self, boxes: np.ndarray, scores: np.ndarray, threshold: float = 0.45
    ):
        """
    ...
    def scale_masks(masks: np.ndarray, shape: Tuple[int, int], padding: bool = True):
        ...
        resized_masks = np.zeros(
            (masks_cropped.shape[0], shape[0], shape[1]), dtype=masks_cropped.dtype
        )
        for i, mask in enumerate(masks_cropped):
            resized_masks[i] = cv2.resize(
                mask, (shape[1], shape[0]), interpolation=cv2.INTER_LINEAR
            )
    ...
    def non_max_suppression(
        ...
        if prediction.shape[-1] == 6 or end2end:
            ...
            if classes is not None:
                output = [
                    pred[np.any(pred[:, 5:6] == classes, axis=1)] for pred in output
                ]
            return output
        ...
        if not rotated:
            if in_place:
                ...
                prediction = np.concatenate(
                    (self.xywh2xyxy(prediction[..., :4]), prediction[..., 4:]), axis=-1
                )
