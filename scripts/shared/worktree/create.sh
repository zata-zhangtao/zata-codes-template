...
function ai_worktree() {
    ...
    worktree_run_state_file="$target_abs_path/.env.run-state"
    mkdir -p "$(dirname "$worktree_run_state_file")"
    local random_backend_port random_frontend_admin_port random_frontend_public_port
    random_backend_port=$((8000 + RANDOM % 1000))
    random_frontend_admin_port=$((5180 + RANDOM % 820))
    random_frontend_public_port=$((3010 + RANDOM % 990))
    {
        printf 'BACKEND_PORT=%s\n' "$random_backend_port"
        printf 'FRONTEND_ADMIN_PORT=%s\n' "$random_frontend_admin_port"
        printf 'FRONTEND_PUBLIC_PORT=%s\n' "$random_frontend_public_port"
    } > "$worktree_run_state_file"
    echo "✅ worktree 运行端口：backend=$random_backend_port, admin=$random_frontend_admin_port, public=$random_frontend_public_port"
