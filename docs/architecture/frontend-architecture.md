...
- 它通过 HTTP 接口调用 `backend/api/` 暴露的后端能力。
...
flowchart TD
    ...
    ApiClient --> Backend["backend/api/ HTTP API"]
```
...
```mermaid
flowchart LR
    Frontend["frontend/ Web Client"] -->|HTTP / WebSocket| Apps["backend/api/"]
    Apps --> Core["backend/core/"]
    Core --> Capabilities["backend/engines/"]
...
- 后端内部如何在 `backend/api/`、`backend/core/`、`backend/engines/`、`backend/infrastructure/` 之间拆分，对前端来说应是透明的。
